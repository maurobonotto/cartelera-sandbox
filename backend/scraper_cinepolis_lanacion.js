...
const OUTPUT_FILE = path.join(__dirname, 'peliculas_cinepolis.json');

// Lista de cines Cinépolis
const CINEPOLIS_CINES = [
    {
        slug: 'cinepolis-avellaneda-sa167',
        nombre: 'Cinépolis Avellaneda',
        ciudad: 'Avellaneda'
    },
    {
        slug: 'cinepolis-plaza-houssay-sa1225',
        nombre: 'Cinépolis Plaza Houssay',
        ciudad: 'CABA'
    }
    // Agregar más aquí cuando quieras
];

// Función auxiliar (idéntica a la de Lorca)
...
async function scrapeCinepolis() {
    console.log('🎬 Scraping cines Cinépolis desde La Nación (selector corregido)');
    ...
    for (const cine of CINEPOLIS_CINES) {
        ...
        try {
            ...
            for (let idx = 0; idx < peliculas.length; idx++) {
                ...
                // 2. Obtener los días usando la clase correcta
                ...
                // 3. Iterar sobre cada día
                for (let i = 0; i < diasTitulos.length; i++) {
                    // Hacer clic en el título del día (para activarlo)
                    await diasTitulos[i].click();
                    await new Promise(r => setTimeout(r, 500));

                    // Extraer fecha y horarios del artículo correspondiente
                    // Usamos evaluate con el índice porque el DOM ya tiene los artículos
                    const { fechaTexto, horarios } = await page.evaluate((idx) => {
                        ...
                        if (articulos[idx]) {
                            ...
                            // Buscar todos los horarios dentro del artículo (pueden estar en spans o texto)
                ...
                for (const item of horariosPorDia) {
                    ...
                    for (const horario of item.horarios) {
                        todasLasFunciones.push({
                            id_funcion: `cinepolis_${cine.slug}_${peli.peliculaId}_${fechaLegible.replace(/\//g, '-')}_${horario.replace(':', '')}`,
                            titulo: peli.titulo,
    ...
    console.log(`\n✅ Cinépolis completado. ${todasLasFunciones.length} funciones guardadas en ${OUTPUT_FILE}`);
    await browser.close();
}
