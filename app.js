...
function cargarDirecciones() {
    fetch('backend/direcciones.json')
...
function parsearFechaLegible(texto) {
    ...
    if (match) {
        ...
        const mesesCompletos = { 'enero':0,'febrero':1,'marzo':2,'abril':3,'mayo':4,'junio':5,'julio':6,'agosto':7,'septiembre':8,'octubre':9,'noviembre':10,'diciembre':11 };
...
function llenarFiltrosHome(funcionesCartelera) {
    ...
    const diasOrdenados = Array.from(fechasMap.entries()).sort((a,b)=>a[1]-b[1]).map(([fechaStr])=>fechaStr);
...
function mostrarPeliculasHome(listaPeliculas) {
    ...
    for (const peli of listaPeliculas) {
        if (peli.seccion === "cartelera" && !carteleraMap.has(peli.titulo)) carteleraMap.set(peli.titulo, peli);
        else if (peli.seccion === "proximos" && !proximosMap.has(peli.titulo)) proximosMap.set(peli.titulo, peli);
    }
    ...
    for (const peli of carteleraOrdenada) carteleraGrid.appendChild(crearTarjetaPelicula(peli));
    for (const peli of proximosOrdenada) proximosGrid.appendChild(crearTarjetaPelicula(peli));
...
function abrirDetallePelicula(titulo) {
    ...
    detailPoster.onerror = function() { this.onerror=null; this.src=posterPlaceholder; };
...
function renderizarFuncionesDetalle(funciones, filtroHorario) {
    ...
    for (const cine of cinesOrdenados) {
        ...
        for (const grupo of grupos.values()) {
            ...
            if (filtroHorario) horariosMostrar = horariosMostrar.filter(h => h === filtroHorario);
            ...
            html += `
                <div class="funcion-item">
                    ...
                    <div class="horarios-lista">
                        ${horariosMostrar.map(h => `<span class="horario-tag" style="cursor: default;">${h}</span>`).join('')}
                    </div>
                </div>
            `;
        }
